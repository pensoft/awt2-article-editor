...
CanvasRenderer.prototype.prepare = function(node) {
  try {
    ...
    if (isVlist) {
      ...
      const svgString = node.pathName && path[node.pathName] ? path[node.pathName] : undefined;
      if (svgString) {
        const atom = {
          x: this.x,
          y: this.state.ypos,
...
export function prepare(dom, canvas, options) {
...
export function render(dom, canvas, x, y, options) {
