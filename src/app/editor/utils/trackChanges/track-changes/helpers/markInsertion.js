...
const markInsertion = (tr, from, to, user, date, group, markId, connectedTo, insMarkAttrs) => {
    tr.removeMark(from, to, tr.doc.type.schema.marks.deletion);
    tr.removeMark(from, to, tr.doc.type.schema.marks.insertion);
    const insertionMark = tr.doc.type.schema.marks.insertion.create(insMarkAttrs ? insMarkAttrs : {
        user: user.userId,
    ...
    tr.doc.nodesBetween(from, to, (node, pos) => {
        if (
            pos < from || ['bullet_list', 'ordered_list'].includes(node.type.name)
        ) {
            return true;
        } else if (
            node.isInline || ['table_row', 'table_cell'].includes(node.type.name)
        ) {
            return false;
        }
        if (node.attrs.track) {
