...
let previewMode
export let sherePreviewModeState = (prevObj) => {
    previewMode = prevObj
  }
...
export class MenuItem {
  ...
  render(view) {
    ...
    function update(state) {
      ...
      let enabled = true
      if (spec.enable) {
        enabled = (!previewMode.mode && spec.enable(state) || false)
        setClass(dom, prefix + "-disabled", !enabled)
      } else {
        enabled = !previewMode.mode
        setClass(dom, prefix + "-disabled", !enabled)
      }
...
export class Dropdown {
  ...
  render(view,options2) {
    ...
    let label1 = this.options.icon ? crel("div", {
      ...
      }, getIcon(this.options.icon)) :
      this.options.label ? crel("div", {
        class: prefix + "-dropdown " + (this.options.class || ""),
        style: this.options.css
    ...
    let update = (state) => {
      if (options.dropdownType == 'alignmenu' && view.hasFocus()) {
        let textAt = state.doc.nodeAt(state.selection.from)
        if (textAt && textAt.type.name == 'text') {
          if (textAt.parent && textAt.parent.attrs.align) {
            let al = textAt.parent.attrs.align;
            if (al == "set-align-center") {
              label.innerHTML = getIcon(createCustomIcon('align.svg', 20)).outerHTML
            } else if (al == "set-align-left") {
              label.innerHTML = getIcon(createCustomIcon('align2.svg', 20)).outerHTML
            } else if (al == "set-align-right") {
              label.innerHTML = getIcon(createCustomIcon('align1.svg', 20)).outerHTML
            }
