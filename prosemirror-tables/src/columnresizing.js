...
function updateColumnWidth(view, cell, width,event) {
  let $cell = view.state.doc.resolve(cell);
  let table = $cell.node(-1),
    map = TableMap.get(table),
    start = $cell.start(-1);
  let tableRowPos = undefined
  for(let i = $cell.path.length-3;i>=0;i-=3){
    let parrel = $cell.path[i];
    if(tableRowPos == undefined&&parrel.type.name == 'table_row'){
      tableRowPos = $cell.path[i-1];
    }
  }
  let tableHTMLNode
  let i = 0;
  ...
  for (let row = 0; row < map.height; row++) {
    ...
    let tableAttrs = table.attrs;
    colwidth[index] = width;
    tr.setNodeMarkup(start + pos, null,setAttr(attrs, 'colwidth', colwidth,));
    tr.setNodeMarkup(tableRowPos, null, setAttr(attrs, 'tablewidth', tableWidth));
  }
